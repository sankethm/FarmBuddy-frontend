...
const ConfirmationScreen = () => {
  const steps = [
    ...
    { title: "Payment", content: "Order Summary" },
  ...
  const total = cart
    ?.map((item) => parseInt(item.price) * (1-parseInt(item.discount)/100) * parseInt(item.quantity))
  ...
  const fetchAddresses = async () => {
    ...
    try {
      const response = await axios.get(
        `http://192.168.43.129:8000/addresses/${userId}`,{"headers":{
  ...
  const handlePlaceOrder = async () => {
    try {
      ...
      const response = await axios.post("http://192.168.43.129:8000/orders", orderData,
        {
        "headers":{
        'Content-Type':'application/json'
        }})
    ...
    <ScrollView>
      ...
      {currentStep == 0 && (
        <View style={{ marginHorizontal: 20 }}>
          <Text style={{ fontSize: 16, fontWeight: "bold" }}>Set Delivery Address</Text>
            ...
            {addresses?.map((item, index) => (
              ...
              >
                {selectedAddress && selectedAddress._id === item?._id ? (
                  ...
                  <Entypo onPress={() => setSelectedAddress(item)} name="circle" size={20} color="grey" />
                )}
